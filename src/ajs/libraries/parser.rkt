...
(define (parse-declaration p)
  ...
  (with-handlers ([exn:parse-error? handle-parse-error])
    (cond
      ...
      [(matches? p FUNCTION) (parse-fun-declaration p "function")]
      [(matches? p CONST) (parse-var-declaration p)]
...
(define (parse-parameters p)
  ...
  (unless (check? p RIGHT_PAREN)
    ...
    (while (matches? p COMMA)
           (when (>= paramc 255)
             (ajs-error (peek p) "Can't have more than 255 parameters."))
           (add-param!)))
  ...
  (cond
    [(matches? p EQUAL)
     ...
     (match expr
       ...
       [else
        (ajs-error equals "Invalid assignment target.")
        expr])]
    [else expr]))
...
(define (finish-call p callee)
  ...
  (unless (check? p RIGHT_PAREN)
    ...
    (while (matches? p COMMA)
           (when (>= argc 255)
             (ajs-error (peek p) "Can't have more than 255 arguments."))
           (add-arg!)))
...
(define (synchronize p)
  (advance! p)
  (define keywords (list CLASS FUNCTION CONST FOR IF WHILE PRINT RETURN))
  (until (or (at-end? p)
